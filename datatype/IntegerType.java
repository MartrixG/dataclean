...
public class IntegerType implements MetaType {
	private static String selfType = "Integer";
	private Integer value;
	private Integer limitMin, limitMax;

	/**
	 * 默认值为0, 最大值为32位整数的最大值, 最小值为32位整数的最小值
	 */
	public IntegerType() {
		this.value = 0;
		this.limitMin = Integer.MIN_VALUE;
		this.limitMax = Integer.MAX_VALUE;
	}

	@Override
	/**
	 * 将一个字符串解析成一个数字, 首先删除所有的空格和逗号","接着检查是否符合整数的正则表达式,接着转换成整数
	 */
	...
	public void setFormat(String format) throws IllegalClassFormatException {
		...
		if (min_max[0].length() != 0) {
			...
			if (m.matches()) {
				this.limitMin = Integer.valueOf(min_max[0]);
			} else {
				throw new IllegalClassFormatException();
			}
		}
		if (min_max[1].length() != 0) {
			...
			if (m.matches()) {
				this.limitMax = Integer.valueOf(min_max[1]);
			} else {
				throw new IllegalClassFormatException();
			}
		}
	...
	public void write(DataOutput dataOutput) throws IOException {
		...
		dataOutput.writeInt(this.limitMax);
		dataOutput.writeInt(this.limitMin);
	}

	@Override
	public void readFields(DataInput dataInput) throws IOException {
		this.value = dataInput.readInt();
		this.limitMax = dataInput.readInt();
		this.limitMin = dataInput.readInt();
	}
}
